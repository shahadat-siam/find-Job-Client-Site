...
const AddJobs = () => { 
  ...
  const navigate = useNavigate()
   ...
   const hundleFormSubmit = async (e) => { 
    ...
    const jobsData = {
       jobTitle, deadline,  category, min_price, max_price, description, email, bid_count: 0  } 
    try{
      const {data} = await axios.post(`${import.meta.env.VITE_API_URL}/jobs`, jobsData)
      console.log(data)
      toast.success('successfully save job data')
      navigate('/')
    }catch(error){
      console.log(error)
  ...
  return (
    <div className='flex justify-center items-center min-h-[calc(100vh-306px)] my-12'>
      <section className=' p-2 md:p-6 mx-auto bg-[#f8f8f8] rounded-md shadow-md '>
